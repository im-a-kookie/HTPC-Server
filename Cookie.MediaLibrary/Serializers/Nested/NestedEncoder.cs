...
namespace Cookie.Serializers.Nested
{
    ...
    public class NestedEncoder
    {
        ...
        {
            StringBuilder sb = new();
            string indent = "".PadRight(depth, '\t'); // Generate indentation for current depth.
            ...
            var results = data.Select(x =>
            {
                ...
                if (val[0] == 'l' || val[0] == 'd' || val[0] == 'm')
                {
                    return $"{x.Key}*\n{indent}{{{val[0]}\n{indent}\t{val.Substring(1)}\n{indent}}};";
                }
            ...
            return $"m{sb.ToString().Trim()}"; // Prefix with 'm' to indicate a dictionary.
        }
        ...
        internal static string Condense(Dictionary<string, string> data, int depth)
        {
            ...
            string result = Condense(data.Select(x => $"{x.Key}~{x.Value}").ToList(), depth);

            // Prefix with 'd' to indicate a dictionary.
            return 'd' + result.Substring(1);
        }

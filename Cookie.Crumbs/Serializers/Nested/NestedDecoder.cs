    ...
    public class NestedDecoder
    {
        ...
        public static object Destringify(string value)
        {
            ...
            switch (i)
            {
                ...
                case Hfloat: return double.TryParse(span, out var d) ? d : -1;
                case Hstring: return span.ToString().Replace($"&#{(int)Terminator}{AltTerminator}", $"{Terminator}");
                case Hbyte: return span.ToString().ToBytesBase128();
                case Hcoded: return Encoding.UTF8.GetString(span.ToString().ToBytesBase128());
            }
            return value;
        }
        ...
        public static void Process(string body)
        {
            ...
            int closePos = span.IndexOf(CloseGroup) + 1;
            if (closePos > 0)
            {
                ...
                while (closePos > 0)
                {
                    ...
                    int openPos = span.Slice(0, closePos).LastIndexOf(OpenGroup);
                    if (openPos >= 0)
                    {
                    ...
                    closePos = span.Slice(last.start).IndexOf(CloseGroup);
                    if (closePos < 0) break;
        ...
        private static void PopulateDepthAndClean(Node first, string body, Span<char> span)
        {
            ...
            while (n != null)
            {
                ...
                // Calculate the depth
                if (body[n.start] == OpenGroup) ++depth;
                n.depth = depth;
                if (body[n.end - 1] == CloseGroup) --depth;
                ...
                if (len <= 0 || len == 1 && span[n.start] == Terminator)
                {
        ...
        private static void Process(Node first, string body, Span<char> span)
        {
            ...
            while (n != null)
            {
                ...
                // the stack hierarchy
                if (span[n.start] == Terminator || span[n.start] == Hmap)
                {
                ...
                if (previousNode != null)
                {
                    ...
                    if (previousNode.data is Dictionary<string, object?> dict)
                    {
                        ...
                        else
                        {
                            if (prev[^1] == PropDelim)
                            {
                                int index = prev.Slice(1).LastIndexOf(Terminator) + 3;
        ...
        private static void MergeBackwards(Node n, Node? target, Span<char> span, out string? key)
        {
            ...
            if (span[n.end - 1] == PropDelim)
            {
                int pos = span.Slice(n.start, n.end - n.start).LastIndexOf(Terminator) + 1;
                if (pos > 0)
                {
        ...
        private static object? DeserializeContainer(ReadOnlySpan<char> span)
        {
            ...
            if (prefix == Terminator || prefix == Hmap)
                return DeserializeMapping(span.Slice(1));

            // l or d indicate that this is a simpler list/dictionary string enumeration
            else if (prefix == Hlist || prefix == Hdict)
                return DeserializeListOrDictionary(span.Slice(1), prefix == Hdict);
        ...
        private static Dictionary<string, object?> DeserializeMapping(ReadOnlySpan<char> span)
        {
            ...
            while (pos < span.Length)
            {
                ...
                int next = span.Slice(pos).IndexOf(Terminator);
                ...
                if (!segment.IsEmpty)
                {
                    ...
                    int keyDelimiter = segment.IndexOf(PropDelim);
                    if (keyDelimiter < 0)
        ...
        private static object DeserializeListOrDictionary(ReadOnlySpan<char> span, bool isDictionary)
        {
            ...
            if (isDictionary)
            {
                return results
                    .Select(entry =>
                    {
                        string[] parts = entry.Split(KeyValDelim);
                        if (parts.Length != 2)
